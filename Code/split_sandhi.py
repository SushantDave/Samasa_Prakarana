...
def decode_sequence(input_seq, token_index, encoder_model, decoder_model, reverse_target_char_index, max_decoder_seq_length):
...
def infer_sandhi_split(input_text, model, token_index, max_encoder_seq_length, max_decoder_seq_length):
    latent_dim = 128
    ...
    encoder_input_data = np.zeros((1, max_encoder_seq_length, num_tokens), dtype='float32')
    ...
    decoded_sentence = decode_sequence(encoder_input_data, token_index, encoder_model, decoder_model, reverse_target_char_index, max_decoder_seq_length)
    decoded_sentence = decoded_sentence.strip()
    decoded_sentence = decoded_sentence.strip('$')
    return decoded_sentence

def sandhi_split(input_word):
    ...
    dtest = list(json.loads(fp.read()))
    fp.close()
    output = infer_sandhi_split(input_word, model, token_index, max_encoder_seq_length, max_decoder_seq_length)
    return output

if __name__=="__main__":
    print(sandhi_split(sys.argv[1]))
